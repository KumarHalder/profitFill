...
const Update: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (Object.keys(errors).length > 0) {
            ...
            setLoading(true);
            try {
                const item = await updateJob(formValues);
                setSnackbar(`Job with id: ${item.id} Updated Successfully!`, 'success');
            }
            catch (e) {
                setSnackbar(`Job Update Task Failed!`, 'error');
            }
            finally {
                setLoading(false);
            }
                    ...
                    <Stack spacing={8}>
                        ...
                        <Box width={'100%'}>
                            <Button type={'submit'} fullWidth={true} size="large" variant="contained">
                                {loading ? <CircularProgress color={"inherit"} size={26}/> : <> Update Job</>}</Button>
                        </Box>
                    </Stack>
                </FormControl>
